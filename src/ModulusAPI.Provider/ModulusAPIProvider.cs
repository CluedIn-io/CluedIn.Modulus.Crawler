...
namespace CluedIn.Provider.ModulusAPI
{
    public class ModulusAPIProvider : ProviderBase, IExtendedProviderMetadata
    {
        private readonly IModulusAPIClientFactory _modulusapiClientFactory;

        public ModulusAPIProvider([NotNull] ApplicationContext appContext, IModulusAPIClientFactory modulusapiClientFactory)
            : base(appContext, ModulusAPIConstants.CreateProviderMetadata())
        {
            _modulusapiClientFactory = modulusapiClientFactory;
        }
        ...
        public override async Task<AccountInformation> GetAccountInformation(ExecutionContext context, [NotNull] CrawlJobData jobData, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            ...
            var client = _modulusapiClientFactory.CreateNew(modulusapiCrawlJobData);
        ...
        public string Icon => ModulusAPIConstants.IconResourceName;
        public string Domain { get; } = ModulusAPIConstants.Uri;
        public string About { get; } = ModulusAPIConstants.CrawlerDescription;
        public AuthMethods AuthMethods { get; } = ModulusAPIConstants.AuthMethods;
        public IEnumerable<Control> Properties => null;
        public string ServiceType { get; } = JsonConvert.SerializeObject(ModulusAPIConstants.ServiceType);
        public string Aliases { get; } = JsonConvert.SerializeObject(ModulusAPIConstants.Aliases);
        public Guide Guide { get; set; } = new Guide
        {
            Instructions = ModulusAPIConstants.Instructions,
            Value = new List<string> { ModulusAPIConstants.CrawlerDescription },
            Details = ModulusAPIConstants.Details

        };

        public string Details { get; set; } = ModulusAPIConstants.Details;
        public string Category { get; set; } = ModulusAPIConstants.Category;
        public new IntegrationType Type { get; set; } = ModulusAPIConstants.Type;
    }
}
