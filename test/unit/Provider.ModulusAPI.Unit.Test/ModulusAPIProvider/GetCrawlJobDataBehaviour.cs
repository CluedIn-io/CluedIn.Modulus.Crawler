...
{
    public class GetCrawlJobDataBehaviour : ModulusAPIProviderTest
    {
        ...
        [Theory]
        [InlineAutoData(ModulusAPIConstants.KeyName.ApiKey, nameof(ModulusAPICrawlJobData.ApiKey))]
        ...
        [Theory]
        [InlineAutoData]
        public async Task ModulusAPICrawlJobDataReturned(Dictionary<string, object> dictionary, Guid organizationId, Guid userId, Guid providerDefinitionId)
        {
            Assert.IsType<CluedIn.Crawling.ModulusAPI.Core.ModulusAPICrawlJobData>(
                await Sut.GetCrawlJobData(_context, dictionary, organizationId, userId, providerDefinitionId));
        }
    }
}
